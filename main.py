...
from POC.PeiQi文库.OA产品漏洞.通达OA.通达OA_v11_9任意命令执行漏洞 import POC, fofa_key, bug_name

update_fofa_flag = False
dir_name = os.path.dirname(os.path.abspath(__file__))
if (
    not os.path.exists(f"{dir_name}/FOFA_URL/{bug_name}_url.txt")
    or update_fofa_flag == True
):
    fofa.select(fofa_key)
    fofa.save_file(f"{dir_name}/FOFA_URL/", bug_name + "_url.txt")
    fofa_url_list = [i[0] for i in fofa.get_select_data()]
else:
    with open(
        f"{dir_name}/FOFA_URL/{bug_name}_url.txt", "r", encoding="utf-8"
    ) as fofa_file:
